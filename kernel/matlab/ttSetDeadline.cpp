                  ...
                  int nrhs, const mxArray *prhs[] )
{
  ...
  if (nrhs == 0 || nrhs > 2) {
    TT_MEX_ERROR("ttSetDeadline: Wrong number of input arguments!\nUsage: ttSetDeadline(value) or ttSetDeadline(value, taskname)");
    return;
  }

  if (!mxIsDoubleScalar(prhs[0])) {
    TT_MEX_ERROR("ttSetDeadline: value must be a number");
    return;
  }
  ...
  if (nrhs == 1) {
    ...
    if (mxIsChar(prhs[1]) != 1 || mxGetM(prhs[1]) != 1) {
      TT_MEX_ERROR("ttSetDeadline: taskname must be a string");
      return;
    }
