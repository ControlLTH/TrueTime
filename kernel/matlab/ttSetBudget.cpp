                  ...
                  int nrhs, const mxArray *prhs[] )
{
  ...
  if (nrhs == 0 || nrhs > 2) {
    TT_MEX_ERROR("ttSetBudget: Wrong number of input arguments!\nUsage: ttSetBudget(value) or ttSetBudget(value, taskname)");
    return;
  }

  if (!mxIsDoubleScalar(prhs[0])) {
    TT_MEX_ERROR("ttSetBudget: value must be a number");
    return;
  }
  ...
  if (nrhs == 1) {
    ...
    if (mxIsChar(prhs[1]) != 1 || mxGetM(prhs[1]) != 1) {
      TT_MEX_ERROR("ttSetBudget: taskname must be a string");
      return;
    }
