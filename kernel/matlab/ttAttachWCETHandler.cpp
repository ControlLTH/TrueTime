                  ...
                  int nrhs, const mxArray *prhs[] )
{
  ...
  if (nrhs != 2) {
    TT_MEX_ERROR("ttAttachWCETHandler: Wrong number of input arguments!\nUsage: ttAttachWCETHandler(taskname, handlername)");
    return;
  }

  if (mxIsChar(prhs[0]) != 1 || mxGetM(prhs[0]) != 1) {
    TT_MEX_ERROR("ttAttachWCETHandler: taskname must be a non-empty string");
    return;
  }
  if (mxIsChar(prhs[1]) != 1 || mxGetM(prhs[1]) != 1) {
    TT_MEX_ERROR("ttAttachWCETHandler: handlername must be a non-empty string");
    return;
  }
